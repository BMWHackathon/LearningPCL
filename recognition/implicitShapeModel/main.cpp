...
int main(int argc, char **argv)
{
    ...
    std::cout<<"Number of training clouds: "<<number_of_training_clouds<<std::endl;
    // - 1 to number of training_clouds ?
    for (unsigned int i_cloud = 0; i_cloud < number_of_training_clouds ; i_cloud++)
    {
    ...
    std::cout<<"training clouds size: "<<training_clouds.size()<<std::endl;
    ...
    pcl::features::ISMVoteList<pcl::PointXYZ> vote_list = *ism.findObjects(
        model,
        testing_cloud,
        testing_normals,
        testing_class);
